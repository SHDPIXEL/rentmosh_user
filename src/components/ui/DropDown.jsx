...
const Dropdown = ({ options, selectedOption, onSelect, label }) => {
  ...
  return (
    <div className="relative inline-block" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute left-0 z-10 w-auto mt-5 bg-white rounded-md shadow-md">
          <ul className="p-2 text-sm text-gray-700">
            {options.map((option, index) => (
              <li
                key={index}
                onClick={() => handleSelect(option)}
                className="block whitespace-nowrap px-3 py-2 w-full hover:bg-gray-100 rounded-md cursor-pointer"
              >
                {option}
              </li>
            ))}
